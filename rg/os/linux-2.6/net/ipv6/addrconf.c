 ...
 *
 *	$Id: addrconf.c,v 1.1.1.1.56.1 2012/02/10 06:30:08 simba Exp $
 *
...
static void skb_addrconf_rcv(struct sock *sk, int len)
{
...
static int addconf_broadcast_dad_failed(struct inet6_ifaddr *ifp) {
	...
	if (!addrconf_nl_sk) {
		addrconf_nl_sk = netlink_kernel_create(NETLINK_DAD, 0, skb_addrconf_rcv,THIS_MODULE);
		if (!addrconf_nl_sk) {
...
static struct inet6_dev * ipv6_add_dev(struct net_device *dev)
{
	...
	if (ndev) {
		...
		else if (!strcmp(dev->name,"clink1")) {
			printk("ipv6_add_dev on %s,forwarding set to 0 for IOT\n",dev->name);
			ndev->cnf.forwarding = 0;
		}
#endif
...
#ifdef CONFIG_IOT_DAD
		if ( scope == IFA_LINK ){
			idev->cnf.disable_ipv6 = 0;
			printk(KERN_ALERT "Add a LLA on disabled address,Enable it\n");
		}
		else{
			err = -EACCES;
			goto out2;
...
#if 0
	if (!pinfo->onlink) {
		printk(KERN_NOTICE "According to CPE.2.3 Part C,wouldn't accept this Prefix\n");
		return;
	}
#endif
	/*
						  ...
						  u32 prefered_lft,u32 valid_lft)
{
	...
	if (!IS_ERR(ifp)) {
		...
		addrconf_prefix_route(&ifp->addr, ifp->prefix_len, dev,expires, flags);
		/*
...
#ifdef CONFIG_IOT_DAD
	if(ifp->scope == IFA_LINK && (strcmp(ifp->idev->dev->name,"eth0") == 0 || strcmp(ifp->idev->dev->name,"br0") == 0 ) )
		rand_num = 300;
	printk("==========addrconf_dad_kick: dev[%s] addr[%x:%x:%x:%x] randm[%d] HZ[%d]\n"
	,ifp->idev->dev->name,ntohl(ifp->addr.s6_addr32[0]) ,ntohl(ifp->addr.s6_addr32[1])
	,ntohl(ifp->addr.s6_addr32[2]),ntohl(ifp->addr.s6_addr32[3]),rand_num ,HZ  );
#endif
...
#ifdef CONFIG_IOT_DAD	
	printk("*********ndisc_send_ns: dev[%s] addr[%x:%x:%x:%x] \n"
	,ifp->idev->dev->name,ntohl(ifp->addr.s6_addr32[0]) ,ntohl(ifp->addr.s6_addr32[1])
	,ntohl(ifp->addr.s6_addr32[2]),ntohl(ifp->addr.s6_addr32[3])   );
#endif
...
inet6_rtm_newaddr(struct sk_buff *skb, struct nlmsghdr *nlh, void *arg)
{
	...
	return inet6_addr_add(ifm->ifa_index, pfx, ifm->ifa_prefixlen,preferred_lft,valid_lft);
}
