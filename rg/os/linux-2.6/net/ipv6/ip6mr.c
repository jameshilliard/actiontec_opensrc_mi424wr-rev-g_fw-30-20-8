 ...
 *
 *	Version: $Id: ip6mr.c,v 1.1.1.1.56.1 2012/02/10 06:30:08 simba Exp $
 *
 *	Fixes:
...
#if defined(CONFIG_MIPS_BRCM) /* Added by Actiontec.Marts, for ipv6 ready mcast test */
	if(c == NULL) {
		memset( &origin, 0, sizeof(struct in6_addr));
...
static void ip6mr_queue_xmit(struct sk_buff *skb, struct mfc6_cache *c, int vifi)
{
	...
	if (skb->len > dst_mtu(dst)) {
		...
		printk(KERN_WARNING "\n (vifi is %d, mtu(is %d), full_len(is %d),send ICMPV6_PKT_TOOBIG\n"
			, vifi, dst_mtu(dst), skb->len);
...
int ip6_mr_input(struct sk_buff *skb)
{
	...
	read_lock(&mrt_lock);
#if defined(CONFIG_MIPS_BRCM) /* Added by Actiontec.Marts, for ipv6 ready test 1.1.10.J */
        if( IPV6_ADDR_MC_SCOPE(&(skb->nh.ipv6h->daddr)) < IPV6_ADDR_SCOPE_LINKLOCAL ) 
	{
              cache = NULL;
        }
	else 
