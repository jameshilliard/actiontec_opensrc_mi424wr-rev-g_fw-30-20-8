...
#ifdef CONFIG_IPV6_CAP_NS_DAD
struct cap_nsdad_msg_t{
// 0 NS, 1 NA
	   int type;
...
static struct sock *capns_nl_sk = NULL;
static void skb_capns_rcv(struct sock *sk, int len)
{
	printk(KERN_DEBUG "received sth in skb_capns_rcv\n");
	return;
}

int ndisc_capture_ns_dad(struct sk_buff *skb, int type)
{
	struct nlmsghdr *nlh;
	int ret=0;
	struct sk_buff *skb_out = NULL;
	struct cap_nsdad_msg_t *msg;

	struct ipv6hdr * ipv6h=NULL;
	struct nd_msg *nd_msg = NULL;
	struct icmp6hdr *icmpv6_hdr =NULL;
	struct ethhdr * skb_ether = NULL;

	skb_ether = eth_hdr(skb);
	ipv6h = skb->nh.ipv6h;

	icmpv6_hdr = (struct icmp6hdr *) skb->h.raw;
	nd_msg = (struct nd_msg *)skb->h.raw;
	if( icmpv6_hdr && NDISC_NEIGHBOUR_SOLICITATION != icmpv6_hdr->icmp6_type && NDISC_NEIGHBOUR_ADVERTISEMENT != icmpv6_hdr->icmp6_type)
	{
		printk(KERN_DEBUG"Not NS packets ignore!\n");
		return 0;
	}

	if (!capns_nl_sk) {
		capns_nl_sk = netlink_kernel_create(NETLINK_CAP_NS_DAD, 0, skb_capns_rcv,THIS_MODULE);
		if (!capns_nl_sk) {
				printk(KERN_ERR "Create netlink capns_nl_sk error\n");
				return -1;
			}
	}

	skb_out = alloc_skb(NLMSG_SPACE(sizeof(struct cap_nsdad_msg_t)),GFP_KERNEL);
	if (!skb_out) {
	...
	nlh = NLMSG_PUT(skb_out, 0, 0, NLMSG_DONE, sizeof(struct cap_nsdad_msg_t));
	msg = (struct cap_nsdad_msg_t *)NLMSG_DATA(nlh);

	//NS, target IPv6 address
	ipv6_addr_copy(&msg->target_addr,&nd_msg->target);
	//device name
	strncpy(msg->dev_name,skb->dev->name,31);
	//src MAC address
	memcpy(msg->src_mac,skb_ether->h_source,sizeof(msg->src_mac));
	// NS or NA
	msg->type = type;
	...
	NETLINK_CB(skb_out).dst_pid = 0;  	/* multicast */
	NETLINK_CB(skb_out).dst_group = 1;

	/*multicast the message to all listening processes*/
	ret = netlink_broadcast(capns_nl_sk, skb_out, 0, 1, GFP_KERNEL);

	printk(KERN_DEBUG "Broadcast Capture NS DAD  msg ,ret is %d\n",ret);
	return ret;
nlmsg_failure:
	kfree_skb(skb_out);
	return -EINVAL;	
...
static int send_netlink_message_to_userspace(u32 *);

static void skb_ndisc_rcv(struct sock *sk, int len)
{
...
static int send_netlink_message_to_userspace(u32 *flag) {
	...
	if (!ndisc_nl_sk) {
		ndisc_nl_sk = netlink_kernel_create(NETLINK_RAD, 0, skb_ndisc_rcv,THIS_MODULE);
		if (!ndisc_nl_sk) {
	...
	*intp = *flag;
    ...
    /*multicast the message to all listening processes*/
    ret = netlink_broadcast(ndisc_nl_sk, skb, 0, 1, GFP_KERNEL);
	if (ret < 0) {
